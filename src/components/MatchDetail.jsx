...
/**
 * MatchDetail - unified detail page that supports 'scorecard' lowercase shape.
 */
export default function MatchDetail({ matchId, className = '' }) {
  const [center, setCenter] = useState(null);
  const [scard, setScard] = useState(null); // normalized { innings: [...] } or raw
    ...
    <div className={`${matchDetailStyles.container} ${className}`}>
      <div className={matchDetailStyles.headerContainer}>
        ...
        <div className={matchDetailStyles.headerButtonGroup}>
          {lastUpdated && <div className={matchDetailStyles.lastUpdatedText}>Last: {lastUpdated.toLocaleTimeString()}</div>}
          <button 
            onClick={() => fetchAll()} 
            className={matchDetailStyles.refreshButton} 
            disabled={loading}
          >
          ...
          <button 
            ...
            className={matchDetailStyles.rawToggleButton}
          >
      ...
      {loading ? (
        <div className={matchDetailStyles.loadingContainer}><Loader message="Loading match details..." /></div>
      ) : error ? (
        ...
        <div className={matchDetailStyles.mainGrid}>
          ...
          <aside className={matchDetailStyles.sidebarContainer}>
            <div className={matchDetailStyles.summaryCard}>
              <div className={matchDetailStyles.summaryTitle}>Match summary</div>
              ...
              {center?.match?.venueInfo && (
                <div className={matchDetailStyles.venueText}>
                  {center.match.venueInfo.ground} • {center.match.venueInfo.city}
                </div>
              )}
            </div>
